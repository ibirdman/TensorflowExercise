...
class logistic(object):
    def __init__(self):
        self.W = None
    def train(self,X,y,k,learn_rate = 0.01,num_iters = 5000):
        num_train,num_feature = X.shape
        #init the weight
        self.W = 0.001*np.random.randn(num_feature,k).reshape((-1,k))
        print(self.W)
        loss = []
        
        for i in range(num_iters):
            error,dW = self.compute_loss(X,y,l)
 ...
 #               print('i=%d,error=%f' %(i,error))
    ...
    def output_sm(self, X, yl):
        total = 0
        pl = np.exp(np.dot(X, W[:, yl - 1]))
        num = W.shape[0];
        for i in range(num):
            total = total + np.exp(np.dot(X, W[:,i]))
...
def start_train(sample_data):
    ...
    print(X_train)
    classify = logistic()
    loss = classify.train(X_train,y,2)
    print(classify.W)
